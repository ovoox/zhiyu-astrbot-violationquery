...
class ViolationQueryPlugin(Star):
    ...
    async def _query_second_api(self, code: str):
        """查询后续接口获取最终结果"""
        try:
            async with aiohttp.ClientSession() as session:
                # 第二步：使用code获取ticket和uin
                url_step2 = f"http://api.ocoa.cn/api/qqcwg/login.php?type=1&code={code}"
                async with session.get(url_step2) as response2:
                    if response2.status != 200:
                        return f"第二步查询失败，状态码：{response2.status}"
                    ...
                    if json_data2.get("data") and json_data2["data"].get("ticket") and json_data2["data"].get("uin"):
                        ...
                        url_step3 = f"http://api.ocoa.cn/api/qqcwg/safety.php?type=1&ticket={ticket}"
                        async with session.get(url_step3) as response3:
                            if response3.status != 200:
                                return f"第三步查询失败，状态码：{response3.status}"
                            ...
                            if json_data3.get("openid") and json_data3.get("minico_token"):
                                ...
                                # 第四步：获取最终的违规记录
                                url_step4 = f"http://api.ocoa.cn/api/qqcwg/safety.php?type=2&id={openid}&token={minico_token}&uin={uin}"
                                async with session.get(url_step4) as response4:
                                    if response4.status != 200:
                                        return f"第四步查询失败，状态码：{response4.status}"
                                    
                                    final_result = await response4.text()
                                    full_content = f"{final_result}\n因考虑到霸屏缘故 违规内容只返回十条"
                                    return full_content
                            else:
            ...
            self.context.logger.error(f"API查询链失败: {str(e)}")
    ...
    async def on_group_message(self, event: AstrMessageEvent):
        ...
        if msg == "查违规":
            ...
            final_result = await self._query_second_api(code)
